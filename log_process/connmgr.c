...
void connmgr_listen(int port_number,dplist_t ** buffer,pthread_rwlock_t *lock,dplist_t * room_sensor_map_main,pthread_rwlock_t * lock_log,int  logFIFO[])
{
       ...
       //printf("  Error! revents = %d for fd number %i\n", fds[i].revents,fds[i].fd);
         ...
         sensorPH->fd_num=fds[i].fd;
          ...
          pthread_rwlock_wrlock(lock_log);
                  ...
                  writer(logFIFO,now,2,((sensor* )(dpl_get_element_at_index(list,dpl_get_index_of_element(list,sensorPH))))->sensor_id,0);
         ...
         else
         {
             ...
             pthread_rwlock_wrlock(lock_log);
                 ...
                 // char *msg="Connection to SQL server established";
                  ...
                  writer(logFIFO,now,5,sensor_reading->id,0);
                 // free(msg);
